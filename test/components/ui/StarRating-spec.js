...
describe("<StarRating /> UI Component", () => {
    ...
    describe("Rendering the UI", () => {
        ...
        it("renders a div with the class .star-rating", () => expect(wrapper.find("div.star-rating")).to.have.length(1));

        it("renders the correct number of selected stars", () => expect(wrapper.find(".selectedStar")).to.have.length(4));

        it("selects the correct number of stars", () => expect(wrapper.find(MockStar)).to.have.length(7));

        it("has only one paragraph node", () => expect(wrapper.find("p")).to.have.length(1));

        it("displays {rating} of {total} stars", () => expect(wrapper.find("p").text()).to.equal("4 of 7 stars"));

        it("rating does cause error", () => wrapper.childAt(3).simulate("click"));
